...
/*   Updated: 2024/11/24 13:02:13 by sbat             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_tmpfill(char *str)
{
	...
	tmp = (char *)malloc(i + 1);
	if (!tmp)
		return (perror("ft_tmpfill: malloc error"), NULL);
	i = 0;
	while (str[i] && str[i] != '\n')
	{
		tmp[i] = str[i];
...
char	*ft_reset(char *str)
{
	...
	if (!tmp)
		return (perror("ft_reset: malloc error"), freestr(str));
	while (str[i])
	{
		tmp[j] = str[i];
		i++;
...
char	*get_next_line(int fd)
{
	...
	if (!c)
		return (perror("get_next_line: malloc error"), freestr(str));
